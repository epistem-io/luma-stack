...
class SampleSizeCalculator:
    """
    Calculator for adequate reference sample sizes for stratified random sampling 
    that can be distributed across the strata (class). For reference, see: Cochran, W. G., 1977. Sampling techniques. John Wiley & Sons.
    This class provides helper methods to compute sample sizes and generate
    reference data for accuracy assessment workflows.
    """
    
    def __init__(self):
        ensure_ee_initialized()
    #get pixel counts for each class with retry logic to handle error
    def get_pixel_counts_per_class(
        ...
        #main logic to get pixel counts using earth engine reducers
        for attempt in range(max_retries):
            try:
                ...
                histogram_dict = (histogram.get('classification').getInfo() 
    ...
    def validate_inputs(
        ...
        if not (0.001 <= standard_error <= 0.1):
            errors.append(f"Standard error must be in [0.001, 0.1]")
        
        return len(errors) == 0, errors
    
    #main logic to calculate sample size using Cochran's formula for stratified sampling
    @staticmethod
    def calculate_stratified_sample_size(
        ...
        dict[str, Any]
            ...
            - stratum_std_devs: dict[int, float]
    ...
    #Generate stratified random samples using earth engine built-in methods
    def generate_stratified_samples(
        ...
        # Extract class IDs and sample counts as lists
        ...
        # Add sample_id to each feature
        def add_sample_id(feature):
            return feature.set('sample_id', feature.id())

        samples_with_id = samples.map(add_sample_id)

        return samples_with_id
    #function to export the generated samples as a shapefile.
    #current issue: The class name is not written in the attribute table, only class ID
    #add more option, ie KML export
    def export_samples_to_shapefile(
        self,
        ...
        # Set CRS to WGS84 (EPSG:4326) - standard for Earth Engine
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Export to shapefile (creates .shp, .shx, .dbf, .prj files)
...
#will be rename to ref_data_flagging (?)
class ValidationPointClassifier:
    """Classifier for validation points with error analysis.

    Provides helper methods to flag correct/incorrect predictions and
    generate popups for interactive mapping.
    """
    ...
    def classify_validation_points(
        ...
        """Add correctness classification to validation points.
        ...
        accuracy_results : dict
            Placeholder for compatibility; not currently used.
        class_property : str, optional
            Column name in GeoDataFrame for actual class labels.
        predicted_property : str, optional
            Column name in GeoDataFrame for predicted class labels.
    ...
    def generate_popup_html(
        ...
        """Generate HTML popup for a validation point.
        ...
        -------
        str
            HTML string suitable for popup display.
        """
        ...
        if class_names:
            actual_name = class_names.get(actual_class, f"Class {actual_class}")
            predicted_name = class_names.get(predicted_class, f"Class {predicted_class}")
        else:
            actual_name = f"Class {actual_class}"
        ...
        <div style="font-family: Arial, sans-serif; padding: 10px; min-width: 200px;">
            ...
            <div style="margin: 8px 0;">
                <p style="margin: 5px 0; font-size: 14px;">
                    <strong>Actual:</strong> {actual_name}
                </p>
                <p style="margin: 5px 0; font-size: 14px;">
                    <strong>Predicted:</strong> {predicted_name}
                </p>
            </div>
...
class Thematic_Accuracy_Assessment:
    ...
    def _extract_confusion_matrix_data(
        ...
        -------
        dict[str, Any]
            Dictionary containing overall accuracy, kappa, producer/user accuracies,
            and the confusion matrix array.
        """
        #extract the cofnusion matrix data, including some built-in metrics
        #note:
        #earth engine natively used the term "Consumer accuracy". In Remote Sensing terminology, user accuracy is more commonly used
        #in this function, consumer accuracy is mapped to user accuracy for consistentcy
        try:
    ...
    def run_accuracy_assessment(
        ...
        -------
        tuple[bool, dict]
            (success, results) where results contains accuracy metrics or an error message.
        """
        try:
